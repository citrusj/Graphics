...
void display(void) {
	...
	//Second Modeling Formation
	for (int i = 0; i <= 180; i += 30) {
		float angle = (float)i;
		ModelViewProjectionMatrix = glm::scale(ViewProjectionMatrix, glm::vec3(1.0f, -1.0f, 1.0f));  //figure B.1
		//ModelViewProjectionMatrix = glm::rotate(ViewProjectionMatrix, 180 * TO_RADIAN, glm::vec3(1.0f, 0.0f, 0.0f));  //figure B.2

		ModelViewProjectionMatrix = glm::translate(ModelViewProjectionMatrix, glm::vec3(0.0f, 2.0f, 0.0f));
...
void initialize_OpenGL(void) {
	glClearColor(150 / 255.0f, 150 / 255.0f, 150 / 255.0f, 1.0f); // Gray
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

	glEnable(GL_DEPTH_TEST);

	ViewMatrix = glm::lookAt(glm::vec3(15.0f, 12.0f, 15.0f), glm::vec3(0.0f, 0.0f, 0.0f),
