...
uniform float u_blind_timer = 0.0f;
		...
		if (u_light[i].position.w != zero_f) { // point light source
			...
			if (u_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
				...
				if (tmp_float >= cos(radians(spot_cutoff_angle))) {
					if(!u_blind_effect || i!=1){
						tmp_float = pow(tmp_float, u_light[i].spot_exponent);
					}
					else{
						tmp_float = cos(u_blind_timer*acos(tmp_float));
...
// For a normal rendering ...
	...
	if (screen_effect > 0) {
		...
		if ( ((x_mod*x_mod - x_mod + y_mod*y_mod - y_mod) <= -0.25f ) &&(y_mod<0.5)) discard;
		if(((x_mod*x_mod - (x_mod*0.5f) + y_mod*y_mod - y_mod) <= -0.25f) && (y_mod>=0.5)) discard;
		if(((x_mod*x_mod - (x_mod*1.5f) + y_mod*y_mod - y_mod) <= -0.75f) && (y_mod>=0.5)) discard;
	}
